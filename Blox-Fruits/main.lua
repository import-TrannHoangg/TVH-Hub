...
createToggle(settings, "Tự Động Nhập Code", 147, "AutoCode")        end)
...
createLabel(settings, "Tiện Ích", 120)
createToggle(settings, "Tự Động Nhập Code", 147, "AutoCode")local TweenService = game:GetService("TweenService")
local Players = game:GetService("Players")
local player = Players.LocalPlayer
local guiParent = player:WaitForChild("PlayerGui")
...
local function createDropdown(parent, text, yPos, options)
    ...
    btn.Size = UDim2.new(1, 0, 0, 38)
    btn.BackgroundTransparency = 1
    btn.Text = "  "..text..": "..tostring(_G.Configs.SelectedWeapon)
    btn.TextColor3 = Color3.new(1,1,1)
    btn.Font = Enum.Font.GothamBold
    btn.TextSize = 14
    btn.TextXAlignment = Enum.TextXAlignment.Left
...
createTab("Tab Cài Đặt", 6)CloseBtn.Size = UDim2.new(0, 30, 0, 30); CloseBtn.Position = UDim2.new(1, -35, 0, 5)
CloseBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50); CloseBtn.Text = "X"; CloseBtn.TextColor3 = Color3.new(1, 1, 1)
CloseBtn.Font = Enum.Font.SourceSansBold; CloseBtn.TextSize = 18; Instance.new("UICorner", CloseBtn).CornerRadius = UDim.new(0, 6)
...
local TabContainer = Instance.new("ScrollingFrame", Sidebar)
TabContainer.Size = UDim2.new(1, 0, 1, -10); TabContainer.BackgroundTransparency = 1; TabContainer.ScrollBarThickness = 0; TabContainer.CanvasSize = UDim2.new(0,0,0,500)

local PagesContainer = Instance.new("Frame", Main)
PagesContainer.Size = UDim2.new(1, -170, 1, -50); PagesContainer.Position = UDim2.new(0, 170, 0, 45); PagesContainer.BackgroundTransparency = 1
...
local function createDropdown(parent, text, yPos, options, callback)
    ...
    dropFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 35); dropFrame.ClipsDescendants = true; dropFrame.ZIndex = 500
    Instance.new("UICorner", dropFrame)
    local btn = Instance.new("TextButton", dropFrame)
...
local function createToggle(parent, text, yPos, configKey)
    ...
    Instance.new("UICorner", tgl)
    local dot = Instance.new("Frame", tgl)
    dot.Size = UDim2.new(0, 20, 0, 20); dot.Position = UDim2.new(1, -32, 0.5, -10); dot.BackgroundColor3 = _G.Configs[configKey] and Color3.new(0, 1, 1) or Color3.new(0.2, 0.2, 0.2)
    Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)
    tgl.MouseButton1Click:Connect(function()
        _G.Configs[configKey] = not _G.Configs[configKey]
...
local function createTab(name, index)
    local pg = Instance.new("ScrollingFrame", PagesContainer)
    pg.Size = UDim2.new(1, 0, 1, 0); pg.BackgroundTransparency = 1; pg.Visible = (index == 1)
    pg.ScrollBarThickness = 2; pg.CanvasSize = UDim2.new(0, 0, 0, 500)
    Pages[name] = pg
    local btn = Instance.new("TextButton", TabContainer)
...
createDropdown(farm, "Công Cụ", 32, {"Melee", "Kiếm", "Trái"}, function(s) _G.Configs.SelectedWeapon = s end)
...
createTab("Tab Điểm Chỉ Số", 4)
createTab("Tab Dịch Chuyển", 5)
createTab("Tab Cài Đặt", 6)                pcall(function()
                end)
                task.wait(0.5)
            end
        end)
...
local Icon = Instance.new("ImageButton", IconGui)
Icon.Size = UDim2.new(0, 55, 0, 55); Icon.Position = UDim2.new(0, 15, 0.5, -27)
Icon.BackgroundColor3 = Color3.fromRGB(30, 30, 30); Icon.Draggable = true; Icon.Active = true
Icon.ClipsDescendants = true
Instance.new("UICorner", Icon).CornerRadius = UDim.new(1, 0)

local GithubLogo = Instance.new("ImageLabel", Icon)
GithubLogo.Size = UDim2.new(1, 0, 1, 0); GithubLogo.Position = UDim2.new(0.5, 0, 0.5, 0)
GithubLogo.AnchorPoint = Vector2.new(0.5, 0.5); GithubLogo.BackgroundTransparency = 1
GithubLogo.Image = "rbxassetid://107925154921311"; GithubLogo.ZIndex = 2

local Stroke = Instance.new("UIStroke", Icon)
Stroke.Thickness = 2; Stroke.Color = Color3.fromRGB(0, 255, 255)

local MainGui = Instance.new("ScreenGui", guiParent)
MainGui.Name = "MainGui"; MainGui.ResetOnSpawn = false; MainGui.Enabled = true
...
lb("Lựa Chọn Công Cụ", 5)
createDropdown(farm, "Công Cụ", 32, {"Melee", "Kiếm", "Trái"}, function(s) 
    _G.Configs.SelectedWeapon = s 
end)
...
createTab("Tab Điểm Chỉ Số", 4)
createTab("Tab Dịch Chuyển", 5)
createTab("Tab Cài Đặt", 6)                task.wait(0.5)
            end
        end)
    end
end
